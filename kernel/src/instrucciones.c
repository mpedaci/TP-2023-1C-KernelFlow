...
bool execute_create_segment(uint32_t segment_size, uint32_t segment_id, t_pcb *pcb)
{
    ...
    t_package* package = get_package(modules_client->memory_client_socket, logger_main);
    ...
    switch(package->operation_code){
        ...
        case COMPACTAR:
            compactar(pcb);
            t_package*  p = get_package(modules_client->memory_client_socket, logger_main);
...
bool execute_fread(char* nombre_archivo, int direccion_logica, int cantidad_bytes, t_pcb* pcb){
}
