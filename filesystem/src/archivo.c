...
void initialize_filesystem( t_config_filesystem *config, t_log *logger_main ){ //esta este con el logger aux para probar las funciones nomas
...
int truncate_file(t_config_filesystem *config, int nuevo_tamanio, char *nombre, t_log *logger_main){
    ...
    if(fcb->puntero_indirecto!=0){
        punterosIndirectos=obtenerPunterosIndirectos(fcb,logger_main);
    }
    if(nuevo_tamanio > fcb->tamanio_archivo){ //CASO EXPANDIR TAMAÑO
        ...
        if(bloques_Adicionales > 0){
            ...
            if(fcb->puntero_indirecto==0){
                bloque_libre= obtener_bloque_libre(bitmap,bitmap_size,logger_main);//bloque directo
                fcb->puntero_directo=bloque_libre;
                if(bloques_Adicionales>1){
                    bloque_libre_puntero_indirecto=obtener_bloque_libre(bitmap,bitmap_size,logger_main);
                    ...
                    for(i=0 ; i < bloques_Adicionales-1; i++ ){
                        bloque_libre= obtener_bloque_libre(bitmap,bitmap_size,logger_main);
...
int obtener_bloque_libre(t_bitarray *bitmap, int bitmap_size,t_log *logger_main){ //obtengo el bloque libre y asigno como ocupado en el bitmap
...
int* obtenerPunterosIndirectos(t_fcb* fcb,t_log* logger_main){
    ...
    int j=0;
    while (j*sizeof(int)<pSuperbloque->block_size){ // 16 veces puede iterar para blocksize 64 
...
/*-----------------------------------------------------------------------------------*/
    ...
    */
    ...
    if(numero_de_bloque_inicial==0){
        ...
        numero_de_bloque_inicial-=1; //original 
        for(int i=numero_de_bloque_inicial; i < numero_de_bloque_inicial + cantida_de_bloques_a_escribir; i++){  //asi me aseguro q itere la cant de veces de cantbloquesaescribir
